...
import ResourceSearchBar from "@/components/resource/ResourceSearchBar"
...
function ResourcesDashboard(){
  ...
  const handleDeleteSelectedBtnClick = () => {
    if (selectedList.length == 0) {
      addNotification({
        content:"resource.no-files-selected"
      })
    } else {
      const warningText = "resource.warning-text"
    //   showCommonDialog({
    //     title: t("resource.delete-resource"),
    //     content: warningText,
    //     style: "warning",
      ...
      await resourceStore.createResourcesWithBlob(e.dataTransfer.files).then(
        (res) => {
          for (const resource of res) {
            addNotification({
                content: `${resource.filename} ${("resource.upload-successfully")}`
            })
          }
        },
        (reason) => {
        ...
        {dragActive && (
          ...
          >
            <div className="flex h-full w-full">
              <p className="m-auto text-2xl text-white dark:text-black">{("resource.file-drag-drop-prompt")}</p>
            </div>
          </div>
        )}
        ...
        <div className="w-full flex flex-col justify-start items-start px-4 py-3 rounded-xl bg-white dark:bg-zinc-700 text-black dark:text-gray-300">
          <div className="relative w-full flex flex-row justify-between items-center">
            <p className="flex flex-row justify-start items-center select-none rounded">
              <Icon.Paperclip className="w-5 h-auto mr-1" /> {("common.resources")}
            </p>
          ...
          <div className="w-full flex flex-col justify-start items-start mt-4 mb-6">
            {loadingState.isPending ? (
              <div className="w-full h-32 flex flex-col justify-center items-center">
                <p className="w-full text-center text-base my-6 mt-8">{("resource.fetching-data")}</p>
              </div>
            ) : (
              <div
                className={
              ...
              >
                {listStyle === "TABLE" && (
                  <div className="px-2 py-2 w-full grid grid-cols-10 border-b dark:border-b-zinc-600">
                    ...
                    <span className="col-span-6">{("common.name")}</span>
                    <span></span>
                  </div>
                )}
                {resourceList.length === 0 ? (
                  <p className="w-full text-center text-base my-6 mt-8">{("resource.no-resources")}</p>
                ) : (
                  resourceList
                )}
              </div>
