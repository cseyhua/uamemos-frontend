...
import { getResourceUrl } from "@/helper/utils"
...
const ResourceItemDropdown = ({ resource }: Props) => {
  ...
  const handleCopyResourceLinkBtnClick = (resource: Resource) => {
    ...
    copy(url);
    toast.success(("message.succeed-copy-resource-link"));
  };
  ...
  const handleDeleteResourceBtnClick = (resource: Resource) => {
    let warningText = ("resource.warning-text");
    if (resource.linkedMemoAmount > 0) {
      warningText = warningText + `\n${("resource.linked-amount")}: ${resource.linkedMemoAmount}`;
    }
