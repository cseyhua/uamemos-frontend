...
import { getMemoStats } from "@/helper/api"
import { DAILY_TIMESTAMP } from "@/helper/consts"
import { getDateStampByDate, getDateString, getTimeStampByDate } from "@/helper/datetime"
import * as utils from "@/helper/utils"
import "./css/usage-heat-map.less"
...
function UsageHeatMap() {
    ...
    const handleUsageStatItemMouseEnter = useCallback((event: React.MouseEvent, item: DailyUsageStat) => {
        ...
        tempDiv.innerHTML = item.count === 1 ? (tMemoOnOpts.amount+" memo on " + tMemoOnOpts.date) : (tMemoOnOpts.amount+" memos on " + tMemoOnOpts.date)
        ...
        <>
            <div className="usage-heat-map-wrapper" ref={containerElRef}>
                ...
                <div className="day-tip-text-container">
                    <span className="tip-text">{("日")}</span>
                    <span className="tip-text"></span>
                    <span className="tip-text">{("二")}</span>
                    <span className="tip-text"></span>
                    <span className="tip-text">{("四")}</span>
                    <span className="tip-text"></span>
                    <span className="tip-text">{("六")}</span>
                </div>
            </div>
